...
def arimax_predictions(row, test_periods, prediction_periods, seasonal_periods,
                        additional_params, model_name: str, error_method, row_exog_data=None, row_exog_projected=None):
    try:
        ...
        if row_exog_data is None:
            ...
            model_name = 'arima'
            model = ARIMA(train_data, order=arima_order, seasonal_order=(0, 0, 0, int(seasonal_periods)))
            ...
            model = ARIMA(df_merged_data['Sales'], order=arima_order, exog=df_merged_data['Variable'],
                          seasonal_order=(0, 0, 0, int(seasonal_periods)))
            ...
            if row_exog_projected is not None:
                ...
                future_predictions = model_fit.forecast(steps=prediction_periods, exog=df_merged_data['Variable'])
